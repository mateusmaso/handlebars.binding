(function(Handlebars) {
  ...
  Handlebars.registerHelper('bind', function(keypath, options) {
    ...
    var react = function() {
      if (!document.body.contains(marker) && !document.body.contains(node)) return observer.close();

      if (options.hash.attr) {
        if (options.hash.attr == "class") {
          Utils.removeClass(node, previousValue);
          if (value) Utils.addClass(node, value); 
    ...
    var observe = function() {
      if (Utils.isArray(value)) {
        return new ArrayObserver(value, function() { react() });
      } else if (Utils.isObject(value)) {
        return new ObjectObserver(value, function() { react() });
      } else {
  ...
  Handlebars.registerHelper('each', function(items, options) {
    ...
    var renderItem = function(item, index) {
      ...
      var observerContext = new ObjectObserver(context, function() {
    ...
    var react = function(splice) {
      if (!document.body.contains(marker)) return observer.close();
      ...
      if (splice.addedCount > 0) {
        for (var index = splice.index; index < (splice.index + splice.addedCount); index++) {
          ...
          if (options.hash.single) {
            ...
            Utils.insertAfter(previous, itemNode);
